public class PhysicsSolver {
    ...
    public static class Slot {
       ...
       private boolean hasVal;
        ...
        public double getValue() {
            if (this.hasVal)return this.val;
        ...
        public boolean setValue(double value) {
             if (this.hasVal == false) {
                 this.val = value;
                 this.hasVal = true;
                 return true;
             }else if (hasVal & areSameDoubles(this.val,value)){
                 return false;
             }
             else{
    ...
    public static class Squaring extends Relation {
        ...
        @Override
        public boolean update() {
            if (number_.hasVal && square_.hasVal) {
                if (Slot.areSameDoubles(square_.val, number_.getValue() * number_.getValue())) {
                    return false;
                } else {
                    square_.setValue(number_.getValue() * number_.getValue());
                    return true;
                }
            } else if (number_.hasVal && !square_.hasVal) {
                square_.setValue(number_.getValue() * number_.getValue());
                return true;
            }else if(!number_.hasVal && square_.hasVal)
                if (Slot.areSameDoubles(0,square_.val)){
                    number_.setValue(0);return true;
                } else {number_.setValue(Math.sqrt(square_.getValue()));return true;}
            return false;
        }
