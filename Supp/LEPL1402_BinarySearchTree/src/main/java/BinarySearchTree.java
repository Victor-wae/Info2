...
public class BinarySearchTree {
    ...
    public List<Integer> decreasing() {
        ...
        decreasingHelper(this.root,linkedList);
        return linkedList;
    }

    private void decreasingHelper(Node node,LinkedList<Integer> linkedList) {
        if (node == null)return;
        else {
            decreasingHelper(node.right,linkedList);
            linkedList.add(node.key);
            decreasingHelper(node.left,linkedList);
        }
    }
}
